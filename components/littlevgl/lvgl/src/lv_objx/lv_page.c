    ...
    if(sign == LV_SIGNAL_CHILD_CHG) { /*Automatically move children to the scrollable object*/
        ...
        while(child != NULL) {
            if(lv_obj_is_protected(child, LV_PROTECT_PARENT) == false) {
                ...
                /* Reposition the child to take padding into account (Only if it's on (0;0) or (widht;height)
                 * coordinates now) It's required to keep new the object on the same coordinate if FIT is enabled.*/
...
static void scrl_def_event_cb(lv_obj_t * scrl, lv_event_t event)
{
    ...
    if(event == LV_EVENT_PRESSED || event == LV_EVENT_PRESSING || event == LV_EVENT_PRESS_LOST ||
       ...
       event == LV_EVENT_LONG_PRESSED || event == LV_EVENT_LONG_PRESSED_REPEAT || event == LV_EVENT_DRAG_BEGIN ||
       event == LV_EVENT_DRAG_END || event == LV_EVENT_DRAG_THROW_BEGIN) {
