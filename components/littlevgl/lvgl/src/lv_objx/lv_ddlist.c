...
lv_obj_t * lv_ddlist_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    if(copy == NULL) {
        ...
        if(lv_obj_get_base_dir(new_ddlist) == LV_BIDI_DIR_RTL)
            x = lv_obj_get_x(new_ddlist) + lv_obj_get_width(new_ddlist);
        else
            x = lv_obj_get_x(new_ddlist);
    ...
    /*Post draw when the children are drawn*/
        ...
        /*Add a down symbol in ddlist when closed*/
            ...
            if(ext->draw_arrow) {
                ...
                lv_coord_t arrow_width =
                    lv_txt_get_width(LV_SYMBOL_DOWN, (uint16_t)strlen(LV_SYMBOL_DOWN), sel_style->text.font, 0, 0);
...
static void lv_ddlist_refr_width(lv_obj_t * ddlist)
{
    ...
    lv_coord_t x;
    if(lv_obj_get_base_dir(ddlist) == LV_BIDI_DIR_RTL)
        x = lv_obj_get_x(ddlist) + lv_obj_get_width(ddlist);
    else
        x = lv_obj_get_x(ddlist);
    ...
    if(lv_obj_get_base_dir(ddlist) == LV_BIDI_DIR_RTL)
        lv_obj_set_x(ddlist, x - lv_obj_get_width(ddlist));
    else
        lv_obj_set_x(ddlist, x);
