...
void lv_label_get_letter_pos(const lv_obj_t * label, uint16_t char_id, lv_point_t * pos)
{
    ...
    if(new_line_start == byte_id) {
        ...
        uint16_t visual_char_pos =
            lv_bidi_get_visual_pos(&txt[line_start], &mutable_bidi_txt, new_line_start - line_start,
...
uint16_t lv_label_get_letter_on(const lv_obj_t * label, lv_point_t * pos)
{
    ...
    /*Handle Bidi*/
    bool is_rtl;
    logical_pos = lv_bidi_get_logical_pos(&txt[line_start], NULL, txt_len, lv_obj_get_base_dir(label),
                                          lv_txt_encoded_get_char_id(bidi_txt, i), &is_rtl);
