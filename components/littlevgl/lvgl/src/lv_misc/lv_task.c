                ...
                if(((lv_task_t *)LV_GC_ROOT(_lv_task_act))->prio > task_interrupter->prio) {
                    if(lv_task_exec(LV_GC_ROOT(_lv_task_act))) {
                        if(!task_created && !task_deleted) {
                            ...
                            if(_lv_task_act != NULL) task_interrupter = LV_GC_ROOT(_lv_task_act);
...
/**
 * Get ticks for next run
 */
uint32_t lv_task_get_tick_next_run(void)
{
    uint32_t tick_now = lv_tick_get();
    int tick_next     = INT32_MAX;

    lv_task_t * task = lv_ll_get_head(&LV_GC_ROOT(_lv_task_ll));
    while(task) {
        /*We reach priority of the turned off task. There is nothing more to do.*/
        if(task->prio == LV_TASK_PRIO_OFF) {
            break;
        }

        uint32_t tick_next_run = task->last_run + task->period;
        int delta              = tick_next_run - tick_now;
        if(delta < tick_next) tick_next = delta;

        task = lv_ll_get_next(&LV_GC_ROOT(_lv_task_ll), task);
    }

    return (tick_next < 0) ? 0 : tick_next;
}
/**
