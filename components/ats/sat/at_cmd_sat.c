...
void at_bip_response(uint8_t nError, CFW_SIM_ID nSimID)
{
    ...
    if (bip_command == SIM_SAT_OPEN_CHANNEL_COM)
    {
        ...
        CFW_GprsGetReqQos(bip_cid, &channel_info.Qos, nSimID);
...
void at_bipSetDefaultQos(CFW_SIM_ID nSimID)
{
    CFW_GPRS_QOS Qos;
    memset(&Qos, 0x00, sizeof(CFW_GPRS_QOS));

    Qos.nPrecedence = 0x03;
    Qos.nDelay = 0x04;
    Qos.nReliability = 0x02;
    Qos.nPeak = 0x09;
    Qos.nMean = 0x1F;
...
void stk_tms_timeout(void *param)
{
    ...
    OSI_LOGI(0, "Timer management: orignal time value = %X", tms->tms_value);

    uint64_t time_value = (uint64_t)osiUpTime();
    OSI_LOGI(0, "Current time value = %X", time_value);

    time_value -= tms->tms_value;
    OSI_LOGI(0, "Timer management: time elapsed = %d", time_value);
...
static void callback_stk_response(atCommand_t *cmd, const osiEvent_t *event)
{
    ...
    struct stk_tms_info *tms = &stk_tms_info[nSim][tms_last_timer_id - 1];
...
#define GCF_27_22_4_21_2_2a
        ...
        {
            CFW_SAT_TERMINAL_ENVELOPE_CNF *p = (CFW_SAT_TERMINAL_ENVELOPE_CNF *)(cfw_event->nParam2);
#ifndef CHIP_HAS_AP
            if (0x90 == (p->Sw1 & 0xFF))
            {
                const char *response = "STNN";
...
#endif
            OSI_LOGI(0x100045ae, "EV_CFW_SAT_RESPONSE_RSP STNN 0x%x p->NbByte %d", cfw_event->nParam1, p->NbByte);
            if (0x00 != p->NbByte)
            {
                /*
...
void _onEV_CFW_SAT_CMDTYPE_IND(const osiEvent_t *event)
{
    ...
    //uint16_t uti = cfwRequestUTI((osiEventCallback_t)NULL, cmd);
    uint16_t uti = cfwRequestNoWaitUTI();
    auto_process = 0xFF;
    int32_t retval = 0;
    uint8_t nCmd = 0;
    ...
    case SIM_SAT_REFRESH_COM:
    {
        ...
        case 0x04:
        {
            ...
            {
                struct stk_tms_info *tms = &stk_tms_info[nSim][i];
    ...
    case SIM_SAT_OPEN_CHANNEL_COM:
    {
        ...
        case OPEN_CHANNEL_BEARER_DEFAULT:
        {
            ...
            {
                CFW_SatResponse(SIM_SAT_OPEN_CHANNEL_COM, 0, 0, &channel_info, sizeof(struct CFW_SAT_CHANNEL), uti, nSim);
    ...
    case SIM_SAT_GET_CHANNEL_STATUS_COM:
    {
        ...
        if ((bip_status >= BIP_SOCKET) && (bip_type == CFW_TCPIP_SOCK_STREAM))
            ...
            nStatus = 0x0005;
        OSI_LOGI(0, "Get GET CHANNEL STATUS!, bip_cid = %d, nStatus = %d", bip_cid, nStatus);
    ...
    case SIM_SAT_TIMER_MANAGEMENT_COM:
    {
        ...
        if ((timer == 0) || (timer > 8))
            goto TIMER_ERROR;
        struct stk_tms_info *tms = &stk_tms_info[nSim][timer - 1];
        ...
        if (nQualifier == 0)
        {
            ...
            uint64_t timer_value = tms->tms_value;
            if ((timer_value != 0) && (timer_value != -1))
            {
            ...
            tms->tms_timer = osiTimerCreate(NULL, stk_tms_timeout, (void *)&timer);
            bool ret = osiTimerStart(tms->tms_timer, timerout * 1000);
            OSI_LOGI(0, "Timerout interval = %d ms", timerout * 1000);
            if (ret == true)
            {
        ...
        else if ((nQualifier == 0x01) || (nQualifier == 0x02))
        {
            ...
            else
            {
                if (nQualifier == 0x01)
                {
                    uint8_t timer_id = timer - 1;
                    if (osiTimerIsRunning(tms->tms_timer))
                    {
                        OSI_LOGI(0, "killed System timer id = %d!", timer_id);
...
#define API_RAT_CAMP_LTE API_RAT_LTE
            ...
            if (rat == API_RAT_CAMP_LTE)
            {
                ...
                memcpy(LocalInfo + 5, status.nCellId, 4);
                length = 9;
            }
            ...
            else
            {
...
#if 0
                retval = CFW_GprsGetstatus(&status, nSim);
                OSI_LOGI(0, "3nCellId = %02X%02X%02X%02X", status.nCellId[0], status.nCellId[1], status.nCellId[2], status.nCellId[3]);
                LocalInfo[5] = status.nCellId[0] << 4 | status.nCellId[1] >> 4;
                LocalInfo[6] = status.nCellId[1] << 4 | status.nCellId[2] >> 4;
                LocalInfo[7] = status.nCellId[2] << 4 | status.nCellId[3] >> 4;
                LocalInfo[8] = status.nCellId[3] << 4 | 0x0F;
                OSI_LOGI(0, "status = %d", status.nStatus);
                if ((status.nStatus == CFW_NW_STATUS_REGISTERED_HOME) ||
                    (status.nStatus == CFW_NW_STATUS_REGISTERED_ROAMING))
                {
                    memcpy(LocalInfo + 5, status.nCellId, 4);
                    length = 9;
                ...
                {
                    retval = CFW_NwGetStatus(&status, nSim);
                    OSI_LOGI(0, "4nCellId = %02X%02X", status.nCellId[0], status.nCellId[1]);
                    if ((status.nStatus == CFW_NW_STATUS_REGISTERED_HOME) ||
                        (status.nStatus == CFW_NW_STATUS_REGISTERED_ROAMING))
                    {
                        memcpy(LocalInfo + 5, status.nCellId, 2);
                    ...
                    {
                        OSI_LOGI(0, "Not in registered status");
                        goto LOCAL_ERROR;
                    }
                }
#endif
        ...
        else if (nQualifier == 0x01)
        {
            ...
            OSI_LOGI(0, "--- IMEIsv ---");
            OSI_LOGXI(OSI_LOGPAR_M, 0, "%c", 15, imei_str);
            uint8_t imei[8];
            uint8_t i = 1;
            uint8_t j = 1;
        ...
        {
            //UINT32 retval = CFW_GetNMR(nUTI, nSim);
            //if(retval != ERR_SUCCESS)
            //    CFW_SatResponse(SIM_SAT_PROVIDE_LOCAL_INFO_COM, 0x20, 0, 0, 0, uti, nSim);
            CFW_SatResponse(SIM_SAT_PROVIDE_LOCAL_INFO_COM, 0x30, 0, 0, 0, uti, nSim);
        }
        ...
        else if (nQualifier == 0x05)
        {
            ...
            if (ERR_SUCCESS != CFW_GetCellInfo(&CellInf, &Dummy, nSim))
                goto LOCAL_ERROR;
            ...
            CFW_SatResponse(SIM_SAT_PROVIDE_LOCAL_INFO_COM, 0, 0, &nTA, sizeof(SAT_TIMINGADV_RSP), uti, nSim);
        }
