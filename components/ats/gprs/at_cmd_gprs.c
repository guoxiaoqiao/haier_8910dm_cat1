    ...
    if (cmd->type == AT_CMD_SET)
    {
        ...
        if (1 <= cmd->param_count)
        {
            cid = atParamDefUintInRange(
                cmd->params[0], AT_PDPCID_MIN, AT_PDPCID_MIN, AT_PDPCID_MAX, &paramok);
...
void atCmdHandleCGDSCONT(atCommand_t *cmd)
{
    ...
    if (AT_CMD_SET == cmd->type)
    {
        ...
        if (cmd->param_count >= 2)
        {
            ...
            int ret = 0;
            uP_Cid_set = atParamDefUintInRange(cmd->params[1], AT_PDPCID_MIN, AT_PDPCID_MIN, AT_PDPCID_MAX, &paramok);
            if (!paramok)
                RETURN_CME_ERR(cmd->engine, ERR_AT_CME_PARAM_INVALID);
...
void atCmdHandleCGTFT(atCommand_t *cmd)
{
    ...
    else if (AT_CMD_TEST == cmd->type)
    {
        ...
        atCmdRespInfoText(cmd->engine, "+CGTFT: (IP), (1-16),(0-255),(0-255).(0-255).(0-255).(0-255).(0-255).(0-255).(0-255).(0-255),"
...
static void _cgactGprsActRsp(atCommand_t *cmd, const osiEvent_t *event)
{
    ...
    CFW_CfgNwGetOperatorName(operatorId, &pOperName, &pOperShortName);
    if (pOperName != NULL && 0 == strcmp("ChinaMobile", (char *)pOperName))
    {
        dm_register(cmd->engine);
    }
#else
    dm_register(cmd->engine);
#endif
...
void atCmdHandleCGCLASS(atCommand_t *cmd)
{
    ...
    if (AT_CMD_SET == cmd->type)
    {
        if (cmd->param_count != 1)
            RETURN_CME_ERR(cmd->engine, ERR_AT_CME_PARAM_INVALID);
        arrCharacterSet = atParamDefStr(cmd->params[0], "B", &paramok);
        if (!paramok)
            RETURN_CME_ERR(cmd->engine, ERR_AT_CME_PARAM_INVALID);
...
void atCmdHandleCGREG(atCommand_t *cmd)
{
    ...
    else if (AT_CMD_READ == cmd->type) // Read command
    {
        ...
        if ((0x00 != sStatus.nStatus) &&
            (0x01 != sStatus.nStatus) &&
            (0x05 != sStatus.nStatus))
            sStatus.nStatus = 0;
        char rsp[64];
...
void atCmdHandleCFGCIOT(atCommand_t *cmd)
{
    ...
    {
        bool paramok = true;
        uint8_t nonip = atParamDefUintInRange(cmd->params[0], CFW_nvGetCiotNonip(), 0, 1, &paramok);
        uint8_t cpciot = atParamDefUintInRange(cmd->params[1], CFW_nvGetCiotCpciot(), 0, 1, &paramok);
        uint8_t upciot = atParamDefUintInRange(cmd->params[2], CFW_nvGetCiotUpciot(), 0, 1, &paramok);
        uint8_t erwopdn = atParamDefUintInRange(cmd->params[3], CFW_nvGetCiotErwopdn(), 0, 1, &paramok);
        uint8_t sms_wo_comb_att = atParamDefUintInRange(cmd->params[4], CFW_nvGetCiotSmsWoCombAtt(), 0, 1, &paramok);
        uint8_t apn_rate_control = atParamDefUintInRange(cmd->params[5], CFW_nvGetCiotApnRateCtrl(), 0, 1, &paramok);
        //  uint8_t epco = atParamDefUintInRange(cmd->params[6], nbiot_nvGetCiotEpco(), 0, 1, &paramok);
        //  uint8_t cpbackoff = atParamDefUintInRange(cmd->params[7], nbiot_nvGetCpBackOffEnable(), 0, 1, &paramok);
        //  uint8_t roam = atParamDefUintInRange(cmd->params[8], nbiot_nvGetCiotRoam(), 0, 1, &paramok);
        if (!paramok || cmd->param_count > 6)
        ...
        CFW_nvSetCiotNonip(nonip);
        CFW_nvSetCiotCpciot(cpciot);
        CFW_nvSetCiotUpciot(upciot);
        CFW_nvSetCiotErwopdn(erwopdn);
        CFW_nvSetCiotSmsWoCombAtt(sms_wo_comb_att);
        CFW_nvSetCiotApnRateCtrl(apn_rate_control);
    ...
    else if (AT_CMD_READ == cmd->type)
    {
        ...
        sprintf(resp, "+CFGCIOT: nonip=%d, cpciot=%d, upciot=%d, erwopdn=%d, "
                ...
                CFW_nvGetCiotCpciot(),
                CFW_nvGetCiotUpciot(),
                CFW_nvGetCiotErwopdn(),
                CFW_nvGetCiotSmsWoCombAtt(),
                CFW_nvGetCiotApnRateCtrl());
        atCmdRespInfoText(cmd->engine, resp);
        RETURN_OK(cmd->engine);
    }
    else if (AT_CMD_TEST == cmd->type)
...
void atCmdHandleCPSMS(atCommand_t *cmd)
{
    ...
    if (AT_CMD_SET == cmd->type)
    {
        ...
        if (cmd->param_count >= 1)
        {
            mode = atParamDefUintInRange(cmd->params[0], CFW_nvGetPsmEnable(), 0, 2, &paramok);
            OSI_LOGI(0, "mode: %d, paramCount: %d,CFW_nvGetPsmEnable():%d", mode, cmd->param_count, CFW_nvGetPsmEnable());
            OSI_LOGI(0, "CFW_nvGetPsmT3412(): %d,CFW_nvGetPsmT3324() :%d", CFW_nvGetPsmT3412(), CFW_nvGetPsmT3324());
            //ATCMDLOG(I, "cpsms mode %d, paramCount %d", mode, cmd->param_count);
            if (!paramok)
                RETURN_CME_ERR(cmd->engine, ERR_AT_CME_PARAM_INVALID);
            ...
            {
            case 0:
                if (CFW_nvGetPsmEnable() != 0)
                {
                    nvWriten |= 0x01; //NVM
                    if (CFW_nvGetPsmEnable() == 1)
                        nvWriten |= 0x80;
                }
                break;
            ...
            case 2:
                ...
                if (CFW_nvGetPsmT3412() != 0xA4)
                {
                    nvWriten |= 0x02;
                    bitmap |= 0x04;
                    T3412 = 0xA4;
                }
                if (CFW_nvGetPsmT3324() != 0x24)
                {
                    nvWriten |= 0x04;
                    bitmap |= 0x08;
                    T3324 = 0x24;
    ...
    else if (AT_CMD_READ == cmd->type)
    {
        uint8_t aucBitStrRead[12] = {0};
        uint8_t aucBitStrRead2[12] = {0};
        AT_Util_ByteToBitStr(&aucBitStrRead[0], CFW_nvGetPsmT3412(), 8);
        AT_Util_ByteToBitStr(&aucBitStrRead2[0], CFW_nvGetPsmT3324(), 8);

        memset(AtRet, 0, sizeof(AtRet));
        uint8_t mode = CFW_nvGetPsmEnable();
...
void atCmdHandleCEDRXS(atCommand_t *cmd)
{
    ...
    if (AT_CMD_SET == cmd->type)
    {
        ...
        if (cmd->param_count >= 1)
        {
            mode = atParamDefUintInRange(cmd->params[0], CFW_nvGetEdrxEnable(), 0, 3, &paramok);
            if (!paramok)
                RETURN_CME_ERR(cmd->engine, ERR_AT_CME_PARAM_INVALID);
            gAtSetting.drx_mode = mode;
            OSI_LOGI(0, "mode: %d, paramCount: %d,CFW_nvGetEdrxEnable():%d", mode, cmd->param_count, CFW_nvGetEdrxEnable());
            OSI_LOGI(0, "CFW_nvGetEdrxValue(): %d", CFW_nvGetEdrxValue());
            switch (mode)
            {
            case 0:
                if (CFW_nvGetEdrxEnable() != 0)
                {
                    nvWriten |= 0x81;
                }
                break;
        ...
        if (cmd->param_count >= 3)
        {
            ...
            if (cmd->params[2]->type != AT_CMD_PARAM_TYPE_EMPTY)
            {
                ...
                if (CFW_nvGetEdrxValue() != edrxValue)
                {
                    nvWriten |= 0x02;
                    if (mode != 0)
                        nvWriten |= 0x80;
        ...
        OSI_LOGI(0, "edrx set success: gAtSetting.drx_mode: %d, mode:%d,edrxValue:%d ",
                 gAtSetting.drx_mode, CFW_nvGetEdrxEnable(), CFW_nvGetEdrxValue());
        RETURN_OK(cmd->engine);
    }
    else if (AT_CMD_READ == cmd->type)
    {
        uint8_t destAucBitStr[8] = {0};
        AT_Util_ByteToBitStr(destAucBitStr, CFW_nvGetEdrxValue(), 4);
        memset(AtRet, 0, sizeof(AtRet));
        sprintf((char *)AtRet, "+CEDRXS: %d,%d,\"%s\"",
                CFW_nvGetEdrxEnable(), g_edrx_type, destAucBitStr);
        atCmdRespInfoText(cmd->engine, (const char *)AtRet);
        RETURN_OK(cmd->engine);
    }
    else if (AT_CMD_TEST == cmd->type)
...
void atCmdHandleCEDRXRDP(atCommand_t *cmd)
{
    if (AT_CMD_EXE == cmd->type)
    {
        ...
        memset(nw_provided_eDrx, 0x00, sizeof(nw_provided_eDrx));
        memset(ptw, 0x00, sizeof(ptw));
        uint8_t mode = CFW_nvGetEdrxEnable();
        OSI_LOGI(0x100000ea, "AT+CEDRXRDP: mode = %d", mode);
        if ((mode == 0) || (mode == 3))
        {
        ...
        {
            AT_Util_ByteToBitStr(requested_eDrx, CFW_nvGetEdrxValue(), 4);
            //  AT_Util_ByteToBitStr(nw_provided_eDrx, gATCurrentNwEdrxValue, 4);
            AT_Util_ByteToBitStr(ptw, CFW_nvGetEdrxPtw(), 4);
...
void atCmdHandleCFGEDRX(atCommand_t *cmd)
{
    ...
    uint8_t edrxPtw = 0;
    uint8_t edrxValue = 0;
    uint8_t ucGetEnable = CFW_nvGetEdrxEnable();
    if (AT_CMD_SET == cmd->type)
    {
        bool paramok = true;
        if (cmd->param_count > 3)
            RETURN_CME_ERR(cmd->engine, ERR_AT_CME_PARAM_INVALID);
        mode = CFW_nvGetEdrxEnable();
        edrxPtw = CFW_nvGetEdrxPtw();
        edrxValue = CFW_nvGetEdrxValue();
        if (cmd->param_count >= 1)
        {
            mode = atParamDefUintInRange(cmd->params[0], CFW_nvGetEdrxEnable(), 0, 1, &paramok);
            ...
            if (cmd->param_count >= 2)
            {
                edrxPtw = atParamDefUintInRange(cmd->params[1], CFW_nvGetEdrxPtw(), 0, 15, &paramok);
                OSI_LOGI(0x100052ba, "AT+CFGEDRX: edrxPtw = %d", edrxPtw);
                if (!paramok)
                    RETURN_CME_ERR(cmd->engine, ERR_AT_CME_PARAM_INVALID);
                if (edrxPtw != CFW_nvGetEdrxPtw())
                {
                    nvWriten |= 0x02;
                    if (mode != 0)
                        nvWriten |= 0x80;
        ...
        if (cmd->param_count >= 3)
        {
            edrxValue = atParamDefUintInRange(cmd->params[2], CFW_nvGetEdrxValue(), 0, 15, &paramok);
        ...
        CFW_GprsSetEdrx(mode, 0, edrxValue, 0);
        CFW_nvSetEdrxPtw(edrxPtw);
        RETURN_OK(cmd->engine);
    }
    else if (AT_CMD_READ == cmd->type)
    {
        ...
        memset(AtRet, 0, sizeof(AtRet));
        sprintf((char *)AtRet, "+CFGEDRX: %d, ptw=%d, edrxVal=%d", ucGetEnable, CFW_nvGetEdrxPtw(), CFW_nvGetEdrxValue());
        atCmdRespInfoText(cmd->engine, (const char *)AtRet);
        RETURN_OK(cmd->engine);
    }
    else if (AT_CMD_TEST == cmd->type)
...
void atCmdHandleVERCTRL(atCommand_t *cmd)
{
    uint32_t errCode = ERR_INVALID_PARAMETER;
    uint8_t ucByte = 0;
    char AtRet[200] = {0}; /* max 20 charactors per cid */
    ...
    {
        sprintf(AtRet, "+VERCTRL: enable=[0-1](0-storeroom; 1-gcf), pdn_auto_attach=[0-1](0-disable;1-enable)");
        atCmdRespInfoText(cmd->engine, AtRet);
        RETURN_OK(cmd->engine);
    }
    else
