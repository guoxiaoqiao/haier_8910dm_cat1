...
void dm_register(struct atCmdEngine *engine)
{
    OSI_LOGI(0, "dm_register");
    if (!(gAtSetting.self_register & 0X01))
    {
        OSI_LOGE(0, "dm_register DM doesn't register");
        return;
    }
...
void atCmdHandleSELFREGISTER(atCommand_t *cmd)
{
    ...
    if (AT_CMD_SET == cmd->type)
    {
        ...
        else
        {
            ...
            {
            case OPERATOR_CMCC:
                gAtSetting.self_register &= 0X06;
                gAtSetting.self_register |= flag;
                break;
            case OPERATOR_UNICOM:
                gAtSetting.self_register &= 0X05;
                gAtSetting.self_register |= (flag << 1);
                break;
            case OPERATOR_CTCC:
                gAtSetting.self_register &= 0X03;
