...
static void _onEV_CFW_CC_SPEECH_CALL_IND(const osiEvent_t *event)
{
    ...
    if (show_ccwa)
    {
        sprintf(rsp, "+CCWA: \"%s\",%d,1,,%d", num_str, call_info->TelNumber.nType,
                call_info->nCLIValidity);
        atCmdRespSimUrcText(sim, rsp);
    }
    if (show_clip)
    {
        sprintf(rsp, "+CLIP: \"%s\",%d,,,,%d", num_str, call_info->TelNumber.nType,
                call_info->nCLIValidity);
        atCmdRespSimUrcText(sim, rsp);
    }
    ...
    if (AT_CMD_SET == cmd->type)
    {
        ...
        else
        {
            ...
            else if (2 == nCmd)
            {
                ...
                ctx->g_cc_chld = active + hold + wait;
...
void atCmdHandleCEVDP(atCommand_t *cmd)
{
    ...
    {
        char response[32] = {0};
        uint8_t getsetting = CFW_ImsGetCevdp();
