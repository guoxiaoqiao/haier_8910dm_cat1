...
osiTimer_t *g_tb_dataStatistics_timer = NULL;
tb_DataStatistics g_tb_DataStatistics[2] = {
    0,
};
...
int tb_data_get_statistics(tb_data_profile_id cid, tb_data_statistics *statistics)
{
    uint8_t i = 0;
    struct tm cur_tm;
    uint32_t notSavedRx = 0;
    uint32_t notSavedTx = 0;
    uint32_t notSavedTime = 0;
    ...
    time_t cur_time = osiEpochSecond();
    ...
    if (g_tb_data_dial_status[cid - 1] == TB_DIAL_STATUS_CONNECTED)
    {
        ...
        CFW_get_Netif_dataCountBySimCid(nSim, TB_PROFILE_ID_PUBLIC, UPLOAD, LWIP_DATA, &size);
        tx_size += size;
        CFW_get_Netif_dataCountBySimCid(nSim, TB_PROFILE_ID_PUBLIC, UPLOAD, PPP_DATA, &size);
        tx_size += size;
        CFW_get_Netif_dataCountBySimCid(nSim, TB_PROFILE_ID_PUBLIC, UPLOAD, RNDIS_DATA, &size);
        tx_size += size;
        g_tb_DataStatistics[cid - 1].prv_tx_bytes = g_tb_DataStatistics[cid - 1].cur_tx_bytes;
        g_tb_DataStatistics[cid - 1].cur_tx_bytes = tx_size;

        CFW_get_Netif_dataCountBySimCid(nSim, TB_PROFILE_ID_PUBLIC, DOWNLOAD, LWIP_DATA, &size);
        rx_size += size;
        CFW_get_Netif_dataCountBySimCid(nSim, TB_PROFILE_ID_PUBLIC, DOWNLOAD, PPP_DATA, &size);
        rx_size += size;
        CFW_get_Netif_dataCountBySimCid(nSim, TB_PROFILE_ID_PUBLIC, DOWNLOAD, RNDIS_DATA, &size);
        rx_size += size;

        statistics->realtime_rx_bytes = rx_size;
        statistics->realtime_tx_bytes = tx_size;
        statistics->realtime_rx_thrpt = g_tb_DataStatistics[cid - 1].cur_rx_bytes - g_tb_DataStatistics[cid - 1].prv_rx_bytes;
        statistics->realtime_tx_thrpt = g_tb_DataStatistics[cid - 1].cur_tx_bytes - g_tb_DataStatistics[cid - 1].prv_tx_bytes;
        statistics->realtime_time = cur_time - g_tb_DataStatistics[cid - 1].act_time;

        OSI_LOGXI(OSI_LOGPAR_D, 0, "realtime_time: %d", statistics->realtime_time);

        notSavedRx = rx_size - g_tb_DataStatistics[cid - 1].saved_rx_bytes;
        notSavedTx = tx_size - g_tb_DataStatistics[cid - 1].saved_tx_bytes;
        notSavedTime = statistics->realtime_time - g_tb_DataStatistics[cid - 1].saved_time;

        OSI_LOGI(0, "notSavedTime: %d", notSavedTime);
    }

    gmtime_r(&cur_time, &cur_tm);

    for (i = 0; i < cur_tm.tm_mday; i++)
    {
        struct tm up_tm;
        gmtime_r(&gAtSetting.tbDataStatistics[cid - 1].up_time[i], &up_tm);
        if (up_tm.tm_year == cur_tm.tm_year && up_tm.tm_mon == cur_tm.tm_mon)
        {
            statistics->monthly_rx_bytes += gAtSetting.tbDataStatistics[cid - 1].rx_bytes[i];
            statistics->monthly_tx_bytes += gAtSetting.tbDataStatistics[cid - 1].tx_bytes[i];
            statistics->monthly_time += gAtSetting.tbDataStatistics[cid - 1].act_time[i];
        }
    }
    statistics->monthly_rx_bytes += notSavedRx;
    statistics->monthly_tx_bytes += notSavedTx;
    statistics->monthly_time += notSavedTime;

    OSI_LOGXI(OSI_LOGPAR_D, 0, "monthly_time: %d", statistics->monthly_time);

    statistics->total_rx_bytes = gAtSetting.tbDataStatistics[cid - 1].total_rx_bytes;
    statistics->total_tx_bytes = gAtSetting.tbDataStatistics[cid - 1].total_tx_bytes;
    statistics->total_time = gAtSetting.tbDataStatistics[cid - 1].total_act_time;
    statistics->total_rx_bytes += notSavedRx;
    statistics->total_tx_bytes += notSavedTx;
    statistics->total_time += notSavedTime;

    OSI_LOGXI(OSI_LOGPAR_D, 0, "total_time: %lld", statistics->total_time);
...
int tb_get_signal_bar_lte(uint8_t nRscp)
{
...
int tb_handle_nw_signal_quality_ind(const CFW_EVENT *cfw_event)
{
...
void tb_IMSItoHomePlmn(uint8_t *InPut, uint8_t *OutPut, uint8_t *OutLen)
{
    uint8_t i;
...
int tb_handle_nw_status_ind(const CFW_EVENT *cfw_event)
{
    uint8_t mode;
    uint8_t OperatorId[6];
    uint8_t nIMSI[15] = {
...
uint8_t tb_data_is_disconnected(void)
{
    uint8_t i;
    for (i = 0; i < sizeof(g_tb_data_dial_status) / sizeof(tb_data_dial_status); i++)
    {
        if (g_tb_data_dial_status[i] != TB_DIAL_STATUS_DISCONNECTED)
        {
            OSI_LOGI(0, "tb_data_is_disconnected g_tb_data_dial_status[%d]:%d", i, g_tb_data_dial_status[i]);
            return 0;
        }
    }
    OSI_LOGI(0, "tb_data_is_disconnected return 1");
    return 1;
}

void tb_dataStatistics_update(tb_data_profile_id cid)
{
    uint8_t nSim = 0;
    uint32_t size = 0;
    uint32_t rx_size = 0;
    ...
    if (TB_PROFILE_ID_PUBLIC != cid && TB_PROFILE_ID_PRIVATE != cid)
    {
        return;
    }
    ...
    OSI_LOGI(0, "tb_dataStatistics_update,g_tb_data_dial_status[%d]: %d", cid - 1, g_tb_data_dial_status[cid - 1]);

    if (g_tb_data_dial_status[cid - 1] == TB_DIAL_STATUS_CONNECTED || g_tb_data_dial_status[cid - 1] == TB_DIAL_STATUS_DISCONNECTING)
    {
        ...
        if ((cur_time - g_tb_DataStatistics[cid - 1].act_time - g_tb_DataStatistics[cid - 1].saved_time >= TB_DATA_STATISTICS_TIME_THRESHOLD) || g_tb_data_dial_status[cid - 1] == TB_DIAL_STATUS_DISCONNECTING)
        {
            time_t saved_act_time;
            struct tm cur_tm;
            struct tm act_tm;
            struct tm update_tm;

            saved_act_time = g_tb_DataStatistics[cid - 1].act_time + g_tb_DataStatistics[cid - 1].saved_time;
            gmtime_r(&saved_act_time, &act_tm);
            gmtime_r(&cur_time, &cur_tm);

            OSI_LOGI(0, "cur_tm.tm_mday is %d, act_tm.tm_mday is %d", cur_tm.tm_mday, act_tm.tm_mday);

            if (cur_tm.tm_mday - act_tm.tm_mday != 0)
            {
                struct tm prv_tm;

                time_t notSavedTime = (cur_time - g_tb_DataStatistics[cid - 1].saved_time);
                time_t curDayTime = cur_tm.tm_hour * 3600 + cur_tm.tm_min * 60 + cur_tm.tm_sec;

                time_t prvTime = cur_time - curDayTime - 3600;

                gmtime_r(&prvTime, &prv_tm);
                if (notSavedTime > curDayTime)
                {
                    gmtime_r(&(gAtSetting.tbDataStatistics[cid - 1].up_time[prv_tm.tm_mday - 1]), &update_tm);

                    if (update_tm.tm_year != prv_tm.tm_year || update_tm.tm_mon != prv_tm.tm_mon || update_tm.tm_mday != prv_tm.tm_mday)
                    {
                        gAtSetting.tbDataStatistics[cid - 1].act_time[prv_tm.tm_mday - 1] = 0;
                        gAtSetting.tbDataStatistics[cid - 1].rx_bytes[prv_tm.tm_mday - 1] = 0;
                        gAtSetting.tbDataStatistics[cid - 1].tx_bytes[prv_tm.tm_mday - 1] = 0;
                        gAtSetting.tbDataStatistics[cid - 1].up_time[prv_tm.tm_mday - 1] = prvTime;
                    }

                    gAtSetting.tbDataStatistics[cid - 1].act_time[prv_tm.tm_mday - 1] += (notSavedTime - curDayTime);

                    OSI_LOGI(0, "nv 1 act_time[%d] is %d", prv_tm.tm_mday, gAtSetting.tbDataStatistics[cid - 1].act_time[prv_tm.tm_mday - 1]);
                }

                gmtime_r(&(gAtSetting.tbDataStatistics[cid - 1].up_time[cur_tm.tm_mday - 1]), &update_tm);

                if (update_tm.tm_year != cur_tm.tm_year || update_tm.tm_mon != cur_tm.tm_mon || update_tm.tm_mday != cur_tm.tm_mday)
                {
                    gAtSetting.tbDataStatistics[cid - 1].act_time[cur_tm.tm_mday - 1] = 0;
                    gAtSetting.tbDataStatistics[cid - 1].rx_bytes[cur_tm.tm_mday - 1] = 0;
                    gAtSetting.tbDataStatistics[cid - 1].tx_bytes[cur_tm.tm_mday - 1] = 0;
                    gAtSetting.tbDataStatistics[cid - 1].up_time[cur_tm.tm_mday - 1] = cur_time;
                }

                gAtSetting.tbDataStatistics[cid - 1].act_time[cur_tm.tm_mday - 1] += curDayTime;

                OSI_LOGI(0, "nv 2 act_time[%d] is %d", cur_tm.tm_mday, gAtSetting.tbDataStatistics[cid - 1].act_time[cur_tm.tm_mday - 1]);
            }
            else
            {
                gmtime_r(&(gAtSetting.tbDataStatistics[cid - 1].up_time[cur_tm.tm_mday - 1]), &update_tm);
                ...
                if (update_tm.tm_year != cur_tm.tm_year || update_tm.tm_mon != cur_tm.tm_mon || update_tm.tm_mday != cur_tm.tm_mday)
                {
                    OSI_LOGI(0, "reset day: %d", cur_tm.tm_mday);
                    gAtSetting.tbDataStatistics[cid - 1].act_time[cur_tm.tm_mday - 1] = 0;
                    gAtSetting.tbDataStatistics[cid - 1].rx_bytes[cur_tm.tm_mday - 1] = 0;
                    gAtSetting.tbDataStatistics[cid - 1].tx_bytes[cur_tm.tm_mday - 1] = 0;
                }
                ...
                gAtSetting.tbDataStatistics[cid - 1].act_time[cur_tm.tm_mday - 1] += (cur_time - g_tb_DataStatistics[cid - 1].act_time - g_tb_DataStatistics[cid - 1].saved_time);
                OSI_LOGI(0, "nv 3 act_time[%d] is %d", cur_tm.tm_mday, gAtSetting.tbDataStatistics[cid - 1].act_time[cur_tm.tm_mday - 1]);
            }

            gAtSetting.tbDataStatistics[cid - 1].total_act_time += (cur_time - g_tb_DataStatistics[cid - 1].act_time - g_tb_DataStatistics[cid - 1].saved_time);

            OSI_LOGXI(OSI_LOGPAR_D, 0, "nv total_act_time is %lld", gAtSetting.tbDataStatistics[cid - 1].total_act_time);

            g_tb_DataStatistics[cid - 1].saved_time = cur_time - g_tb_DataStatistics[cid - 1].act_time;

            OSI_LOGXI(OSI_LOGPAR_D, 0, "saved_time is %lld", g_tb_DataStatistics[cid - 1].saved_time);

            gAtSetting.tbDataStatistics[cid - 1].tx_bytes[cur_tm.tm_mday - 1] += (tx_size - g_tb_DataStatistics[cid - 1].saved_tx_bytes);
            gAtSetting.tbDataStatistics[cid - 1].total_tx_bytes += (tx_size - g_tb_DataStatistics[cid - 1].saved_tx_bytes);

            g_tb_DataStatistics[cid - 1].saved_tx_bytes = tx_size;

            gAtSetting.tbDataStatistics[cid - 1].rx_bytes[cur_tm.tm_mday - 1] += (rx_size - g_tb_DataStatistics[cid - 1].saved_rx_bytes);
            gAtSetting.tbDataStatistics[cid - 1].total_rx_bytes += (rx_size - g_tb_DataStatistics[cid - 1].saved_rx_bytes);

            g_tb_DataStatistics[cid - 1].saved_tx_bytes = rx_size;

            OSI_LOGI(0, "tx_size is %d, rx_size is %d", tx_size, rx_size);
...
void tb_dataStatistics_timer_handler(void *timer)
{
    if (g_tb_data_dial_status[TB_PROFILE_ID_PUBLIC - 1] == TB_DIAL_STATUS_CONNECTED || g_tb_data_dial_status[TB_PROFILE_ID_PUBLIC - 1] == TB_DIAL_STATUS_DISCONNECTING)
        tb_dataStatistics_update(TB_PROFILE_ID_PUBLIC);
    if (g_tb_data_dial_status[TB_PROFILE_ID_PRIVATE - 1] == TB_DIAL_STATUS_CONNECTED || g_tb_data_dial_status[TB_PROFILE_ID_PRIVATE - 1] == TB_DIAL_STATUS_DISCONNECTING)
        tb_dataStatistics_update(TB_PROFILE_ID_PRIVATE);

    osiTimerStart(g_tb_dataStatistics_timer, 1000);
}

void tb_start_data_statistics_timer(tb_data_profile_id cid)
{
    OSI_LOGI(0, "tb_start_data_statistics_timer");
    if (g_tb_dataStatistics_timer == NULL)
    {
        g_tb_dataStatistics_timer = osiTimerCreate(NULL, tb_dataStatistics_timer_handler, NULL);
        osiTimerSetCallback(g_tb_dataStatistics_timer, OSI_TIMER_IN_ISR, tb_dataStatistics_timer_handler, g_tb_dataStatistics_timer);
        osiTimerStart(g_tb_dataStatistics_timer, 1000);
    }
    memset(&(g_tb_DataStatistics[cid - 1]), 0, sizeof(tb_DataStatistics));
    g_tb_DataStatistics[cid - 1].act_time = osiEpochSecond();
    OSI_LOGXI(OSI_LOGPAR_D, 0, "act_time is %lld", g_tb_DataStatistics[cid - 1].act_time);
}

void tb_stop_data_statistics_timer(tb_data_profile_id cid)
{
    OSI_LOGI(0, "tb_stop_data_statistics_timer");
    if (g_tb_dataStatistics_timer != NULL && tb_data_is_disconnected())
    {
        osiTimerStop(g_tb_dataStatistics_timer);
        osiTimerDelete(g_tb_dataStatistics_timer);
        g_tb_dataStatistics_timer = NULL;
    }
    tb_dataStatistics_update(cid);
}

int tb_handle_Gprs_Act_Rsp(const CFW_EVENT *cfw_event)
{
    //uint8_t nSim = cfw_event->nFlag;
...
int tb_handle_Gprs_Active_Ind(const CFW_EVENT *cfw_event)
{
    //uint8_t nSim = cfw_event->nFlag;
...
int tb_handle_Gprs_DeActive_Ind(const CFW_EVENT *cfw_event)
{
    //uint8_t nSim = cfw_event->nFlag;
...
int tb_handle_Init_ind(const CFW_EVENT *cfw_event)
{
    ...
    case CFW_INIT_STATUS_PBK:
    {
        ...
        CFW_SimGetPbkEntry(CFW_PBK_ON, 0x01, nUti, nSim);
    }
    break;
    default:
        break;
...
int tb_handle_Srv_Init_ind(const CFW_EVENT *cfw_event)
{
    uint8_t nSim = 0;
...
int tb_handle_Get_Provider_Id_Rsp(const CFW_EVENT *cfw_event)
{
...
int tb_handle_Get_PBK_Entry_Rsp(const CFW_EVENT *cfw_event)
{
...
int tb_handle_Sms_Send_Rsp(const CFW_EVENT *cfw_event)
{
...
int tb_handle_Sms_Delete_Rsp(const CFW_EVENT *cfw_event)
{
    uint8_t nType = cfw_event->nType;
...
int tb_handle_Sms_Read_Rsp(const CFW_EVENT *cfw_event)
{
...
int tb_handle_New_Sms_Ind(const CFW_EVENT *pCfwEvent)
{
...
int tb_handle_CC_Initiate_Speech_Call_Rsp(const CFW_EVENT *cfw_event)
{
...
int tb_handle_CC_Accept_Call_Rsp(const CFW_EVENT *cfw_event)
{
    tb_voice_notify_call_state_number_info notify_call_info = {
        0,
    };
...
int tb_handle_CC_HungUp_Call_Rsp(const CFW_EVENT *cfw_event)
{
    tb_voice_notify_call_state_number_info notify_call_info = {
        0,
    };
...
int tb_handle_CC_Alerting_Call_Ind(const CFW_EVENT *cfw_event)
{
    tb_voice_notify_call_state_number_info notify_call_info = {
        0,
    };
...
int tb_handle_CC_Speech_Call_Ind(const CFW_EVENT *pCfwEvent)
{
    int num_len = 0;
...
int tb_handle_CC_Error_Call_Ind(const CFW_EVENT *pCfwEvent)
{
    int num_len = 0;
    char num_str[TEL_NUMBER_MAX_LEN + 2];
...
int tb_handle_cfw_event(const osiEvent_t *event)
{
    ...
    case EV_CFW_GPRS_ACT_RSP:
        OSI_LOGI(0, "tb_handle_cfw_event: EV_CFW_GPRS_ACT_RSP");
        tb_handle_Gprs_Act_Rsp(cfw_event);
        break;
    ...
    case EV_CFW_SIM_GET_PROVIDER_ID_RSP:
        OSI_LOGI(0, "tb_handle_cfw_event: EV_CFW_SIM_GET_PROVIDER_ID_RSP");
        tb_handle_Get_Provider_Id_Rsp(cfw_event);
        break;
    case EV_CFW_SIM_GET_PBK_ENTRY_RSP:
        OSI_LOGI(0, "tb_handle_cfw_event: EV_CFW_SIM_GET_PBK_ENTRY_RSP");
        tb_handle_Get_PBK_Entry_Rsp(cfw_event);
        break;
    ...
    case EV_CFW_SMS_SEND_MESSAGE_RSP:
        OSI_LOGI(0, "tb_handle_cfw_event: EV_CFW_SMS_SEND_MESSAGE_RSP");
        tb_handle_Sms_Send_Rsp(cfw_event);
        break;
    case EV_CFW_SMS_READ_MESSAGE_RSP:
        OSI_LOGI(0, "tb_handle_cfw_event: EV_CFW_SMS_READ_MESSAGE_RSP");
        tb_handle_Sms_Read_Rsp(cfw_event);
        break;
    case EV_CFW_SMS_DELETE_MESSAGE_RSP:
        OSI_LOGI(0, "tb_handle_cfw_event: EV_CFW_SMS_DELETE_MESSAGE_RSP");
        tb_handle_Sms_Delete_Rsp(cfw_event);
        break;
