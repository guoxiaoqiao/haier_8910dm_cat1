...
void halAdiBusBatchChange(volatile uint32_t *reg, uint32_t write_mask, uint32_t write_value, ...)
{
    ...
    while ((uint32_t)reg != 0)
    {
        ...
        OSI_LOGV(0, "ADI change reg/%p mask/%p value/%p",
                 (void *)reg, (void *)write_mask, (void *)write_value);
...
void halAdiBusChange(volatile uint32_t *reg, uint32_t mask, uint32_t value)
{
    halAdiBusBatchChange(reg, mask, value, HAL_ADI_BUS_CHANGE_END);
}
